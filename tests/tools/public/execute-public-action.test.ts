...
describe('executePublicAction tool', () => {
  ...
  it('should return formatted MCP response', async () => {
    ...
    expect(mockClient.post).toHaveBeenCalledWith(`/api/public/action/${input.uuid}/execute`, {
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(executePublicActionDefinition.handler(mockClient, input)).rejects.toThrow(
      'API error',
    );
  });

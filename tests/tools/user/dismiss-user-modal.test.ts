...
describe('dismissUserModal tool', () => {
  it('should return formatted MCP response', async () => {
    ...
    const result = await dismissUserModalDefinition.handler(mockClient, {
      id: 1,
      modal: 'onboarding',
    });
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(
      dismissUserModalDefinition.handler(mockClient, { id: 999, modal: 'onboarding' }),
  ...
  it('should propagate API errors with status codes', async () => {
    ...
    await expect(
      dismissUserModalDefinition.handler(mockClient, { id: 1, modal: 'onboarding' }),

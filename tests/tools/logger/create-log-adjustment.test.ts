...
describe('createLogAdjustment tool', () => {
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(createLogAdjustmentDefinition.handler(mockClient, input)).rejects.toThrow('API error');
  });

  it('should propagate API errors with status codes', async () => {
    const mockClient = createMockClientWithError('post', createApiError('Forbidden', 403));
    await expect(createLogAdjustmentDefinition.handler(mockClient, input)).rejects.toThrow('Forbidden');
  });
});

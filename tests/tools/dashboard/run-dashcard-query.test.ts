...
describe('runDashcardQuery tool', () => {
  it('should return formatted MCP response with query results', async () => {
    ...
    const result = await runDashcardQueryDefinition.handler(mockClient, {
      dashboard_id: 1,
      dashcard_id: 2,
      card_id: 3,
    });
  ...
  it('should pass parameters when provided', async () => {
    const mockClient = createMockClientWithResponse('post', { data: { rows: [] } });
    await runDashcardQueryDefinition.handler(mockClient, {
      dashboard_id: 1,
      dashcard_id: 2,
      card_id: 3,
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(
      runDashcardQueryDefinition.handler(mockClient, {
        dashboard_id: 999,
        dashcard_id: 1,
        card_id: 1,
      }),
  ...
  it('should have correct tool definition metadata', () => {
    ...
    expect(runDashcardQueryDefinition.description).toBe(

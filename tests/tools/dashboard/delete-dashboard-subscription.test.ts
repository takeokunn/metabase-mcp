...
describe('deleteDashboardSubscription tool', () => {
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(deleteDashboardSubscriptionDefinition.handler(mockClient, baseInput)).rejects.toThrow(
      'Failed to delete subscription',
    );
  });

  it('should propagate API errors with status codes', async () => {
    const mockClient = createMockClientWithError('delete', createApiError('Subscription not found', 404));
  ...
  it('should have correct tool definition metadata', () => {
    ...
    expect(deleteDashboardSubscriptionDefinition.inputSchema).toEqual(DeleteDashboardSubscriptionInputSchema);
  });
});

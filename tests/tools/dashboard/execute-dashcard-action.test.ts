...
describe('executeDashcardAction tool', () => {
  it('should return formatted MCP response with action result', async () => {
    ...
    const result = await executeDashcardActionDefinition.handler(mockClient, {
      dashboard_id: 1,
      dashcard_id: 2,
      parameters: { key: 'value' },
    });
    expectMcpContent(result, { success: true });
    expect(mockClient.post).toHaveBeenCalledWith('/api/dashboard/1/dashcard/2/execute', {
      parameters: { key: 'value' },
    });
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(
      executeDashcardActionDefinition.handler(mockClient, {
        dashboard_id: 999,
        dashcard_id: 1,
        parameters: {},
      }),
  ...
  it('should have correct tool definition metadata', () => {
    ...
    expect(executeDashcardActionDefinition.description).toBe(

...
describe('saveDashboardToCollection tool', () => {
  it('should return formatted MCP response with saved dashboard', async () => {
    ...
    const result = await saveDashboardToCollectionDefinition.handler(mockClient, { parent_collection_id: 5, dashboard });
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(saveDashboardToCollectionDefinition.handler(mockClient, { parent_collection_id: 5, dashboard: {} })).rejects.toThrow('Not found');
  });
  it('should propagate API errors with status codes', async () => {
    const mockClient = createMockClientWithError('post', createApiError('Unauthorized', 401));
    await expect(saveDashboardToCollectionDefinition.handler(mockClient, { parent_collection_id: 5, dashboard: {} })).rejects.toThrow('Unauthorized');
  });
  it('should have correct tool definition metadata', () => {
    expect(saveDashboardToCollectionDefinition.name).toBe('save_dashboard_to_collection');
    expect(saveDashboardToCollectionDefinition.description).toBe('Save a dashboard to a specific collection in Metabase');
    expect(saveDashboardToCollectionDefinition.inputSchema).toEqual(SaveDashboardToCollectionInputSchema);
  });
});

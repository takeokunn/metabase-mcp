...
describe('getValidFilterFields tool', () => {
  it('should return formatted MCP response with valid filter fields', async () => {
    ...
    const result = await getValidFilterFieldsDefinition.handler(mockClient, {
      filtered: [1],
      filtering: [2, 3],
    });
    expectMcpContent(result, { 1: [2, 3] });
    expect(mockClient.get).toHaveBeenCalledWith('/api/dashboard/params/valid-filter-fields', {
      filtered: [1],
      filtering: [2, 3],
    });
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(
      getValidFilterFieldsDefinition.handler(mockClient, { filtered: [1], filtering: [2] }),
  ...
  it('should have correct tool definition metadata', () => {
    ...
    expect(getValidFilterFieldsDefinition.description).toBe(

...
describe('getDashboardParamRemapping tool', () => {
  it('should return formatted MCP response with remapping data', async () => {
    ...
    const result = await getDashboardParamRemappingDefinition.handler(mockClient, {
      id: 1,
      param_key: 'category',
    });
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(
      getDashboardParamRemappingDefinition.handler(mockClient, { id: 999, param_key: 'key' }),
    ).rejects.toThrow('Not found');
  });
  it('should propagate API errors with status codes', async () => {
    const mockClient = createMockClientWithError('get', createApiError('Unauthorized', 401));
    await expect(
      getDashboardParamRemappingDefinition.handler(mockClient, { id: 1, param_key: 'key' }),
    ).rejects.toThrow('Unauthorized');
  });
  it('should have correct tool definition metadata', () => {
    expect(getDashboardParamRemappingDefinition.name).toBe('get_dashboard_param_remapping');
    expect(getDashboardParamRemappingDefinition.description).toBe(
      'Get remapping for a parameter of a dashboard in Metabase',
    );
    expect(getDashboardParamRemappingDefinition.inputSchema).toEqual(
      GetDashboardParamRemappingInputSchema,
    );
  });
});

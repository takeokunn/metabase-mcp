...
describe('getVirtualCardTableFks tool', () => {
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(getVirtualCardTableFksDefinition.handler(mockClient, { id: 999 })).rejects.toThrow('Not found');
  });
  it('should propagate API errors with status codes', async () => {
    const mockClient = createMockClientWithError('get', createApiError('Unauthorized', 401));
    await expect(getVirtualCardTableFksDefinition.handler(mockClient, { id: 1 })).rejects.toThrow('Unauthorized');
  });
  it('should have correct tool definition metadata', () => {
    expect(getVirtualCardTableFksDefinition.name).toBe('get_virtual_card_table_fks');
    expect(getVirtualCardTableFksDefinition.description).toBe('Get foreign keys for a virtual card-based table in Metabase');

...
describe('deleteDocument tool', () => {
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(deleteDocumentDefinition.handler(mockClient, { id: 999 })).rejects.toThrow(
      'Not found',
    );
  });
  ...
  it('should propagate API errors with status codes', async () => {
    ...
    await expect(deleteDocumentDefinition.handler(mockClient, { id: 1 })).rejects.toThrow(
      'Unauthorized',
    );
  });

...
describe('updateDocument tool', () => {
  it('should return formatted MCP response', async () => {
    ...
    const result = await updateDocumentDefinition.handler(mockClient, {
      id: 1,
      name: 'Updated Doc',
    });
    expectMcpContent(result, { id: 1, name: 'Updated Doc' });
    expect(mockClient.put).toHaveBeenCalledWith('/api/document/1', {
      name: 'Updated Doc',
      content: undefined,
      archived: undefined,
    });
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(updateDocumentDefinition.handler(mockClient, { id: 999 })).rejects.toThrow(
      'Not found',
    );
  });
  ...
  it('should propagate API errors with status codes', async () => {
    ...
    await expect(updateDocumentDefinition.handler(mockClient, { id: 1 })).rejects.toThrow(
      'Unauthorized',
    );
  });

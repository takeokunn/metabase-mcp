...
describe('getPreviewEmbedDashboardParamValues tool', () => {
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(
      getPreviewEmbedDashboardParamValuesDefinition.handler(mockClient, input),
  ...
  it('should have correct tool definition metadata', () => {
    expect(getPreviewEmbedDashboardParamValuesDefinition.name).toBe(
      'get_preview_embed_dashboard_param_values',
    );
    expect(getPreviewEmbedDashboardParamValuesDefinition.description).toBe(
      'Get values for a parameter of a preview embedded dashboard in Metabase',
    );
    expect(getPreviewEmbedDashboardParamValuesDefinition.inputSchema).toEqual(

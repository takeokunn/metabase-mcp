...
describe('searchPreviewEmbedDashboardParamValues tool', () => {
  ...
  it('should return formatted MCP response', async () => {
    ...
    const result = await searchPreviewEmbedDashboardParamValuesDefinition.handler(
      mockClient,
      input,
    );
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(
      searchPreviewEmbedDashboardParamValuesDefinition.handler(mockClient, input),
  ...
  it('should have correct tool definition metadata', () => {
    expect(searchPreviewEmbedDashboardParamValuesDefinition.name).toBe(
      'search_preview_embed_dashboard_param_values',
    );
    expect(searchPreviewEmbedDashboardParamValuesDefinition.description).toBe(
      'Search values for a parameter of a preview embedded dashboard in Metabase',
    );
    expect(searchPreviewEmbedDashboardParamValuesDefinition.inputSchema).toEqual(

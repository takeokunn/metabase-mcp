...
describe('getEntityRevision tool', () => {
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(
      getEntityRevisionDefinition.handler(mockClient, { entity: 'card', id: 999 }),
  ...
  it('should propagate API errors with status codes', async () => {
    ...
    await expect(
      getEntityRevisionDefinition.handler(mockClient, { entity: 'card', id: 1 }),
    ).rejects.toThrow('Unauthorized');
  });

  it('should have correct tool definition metadata', () => {
    expect(getEntityRevisionDefinition.name).toBe('get_entity_revision');
    expect(getEntityRevisionDefinition.description).toBe(

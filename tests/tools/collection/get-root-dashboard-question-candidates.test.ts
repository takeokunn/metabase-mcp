...
describe('getRootDashboardQuestionCandidates tool', () => {
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(
      getRootDashboardQuestionCandidatesDefinition.handler(mockClient, {}),
  ...
  it('should have correct tool definition metadata', () => {
    expect(getRootDashboardQuestionCandidatesDefinition.name).toBe(
      'get_root_dashboard_question_candidates',
    );
    expect(getRootDashboardQuestionCandidatesDefinition.description).toBe(
      'Get dashboard question candidates from the root collection in Metabase',
    );
    expect(getRootDashboardQuestionCandidatesDefinition.inputSchema).toEqual(
      GetRootDashboardQuestionCandidatesParamsSchema,
    );
  });
});

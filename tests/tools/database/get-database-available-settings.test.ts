...
describe('getDatabaseAvailableSettings tool', () => {
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(getDatabaseAvailableSettingsDefinition.handler(mockClient, { id: 999 })).rejects.toThrow('Not found');
  });

  it('should propagate API errors with status codes', async () => {
    const mockClient = createMockClientWithError('get', createApiError('Unauthorized', 401));
    await expect(getDatabaseAvailableSettingsDefinition.handler(mockClient, { id: 1 })).rejects.toThrow('Unauthorized');
  });

  it('should have correct tool definition metadata', () => {
    expect(getDatabaseAvailableSettingsDefinition.name).toBe('get_database_available_settings');
    expect(getDatabaseAvailableSettingsDefinition.description).toBe('Get available settings for a database connection in Metabase');
    expect(getDatabaseAvailableSettingsDefinition.inputSchema).toEqual(GetDatabaseAvailableSettingsParamsSchema);
  });
});

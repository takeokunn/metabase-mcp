...
describe('addRecentActivity tool', () => {
  it('should return formatted MCP response', async () => {
    ...
    const result = await addRecentActivityDefinition.handler(mockClient, {
      model_type: 'card',
      model_id: 1,
    });
    expectMcpContent(result, { id: 1 });
    expect(mockClient.post).toHaveBeenCalledWith('/api/activity/recents', {
      model_type: 'card',
      model_id: 1,
    });
  });
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(
      addRecentActivityDefinition.handler(mockClient, { model_type: 'card', model_id: 999 }),
  ...
  it('should propagate API errors with status codes', async () => {
    ...
    await expect(
      addRecentActivityDefinition.handler(mockClient, { model_type: 'card', model_id: 1 }),
    ).rejects.toThrow('Unauthorized');
  });

  it('should have correct tool definition metadata', () => {
    expect(addRecentActivityDefinition.name).toBe('add_recent_activity');
    expect(addRecentActivityDefinition.description).toBe(

...
describe('getCardParamRemapping tool', () => {
  it('should return formatted MCP response with remapping data', async () => {
    ...
    const result = await getCardParamRemappingDefinition.handler(mockClient, {
      card_id: 1,
      param_key: 'category',
    });
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(
      getCardParamRemappingDefinition.handler(mockClient, { card_id: 999, param_key: 'key' }),
    ).rejects.toThrow('Not found');
  });
  it('should propagate API errors with status codes', async () => {
    const mockClient = createMockClientWithError('get', createApiError('Unauthorized', 401));
    await expect(
      getCardParamRemappingDefinition.handler(mockClient, { card_id: 1, param_key: 'key' }),
    ).rejects.toThrow('Unauthorized');
  });
  it('should have correct tool definition metadata', () => {
    expect(getCardParamRemappingDefinition.name).toBe('get_card_param_remapping');
    expect(getCardParamRemappingDefinition.description).toBe(

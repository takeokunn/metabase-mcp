...
describe('getPersistedModel tool', () => {
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(getPersistedModelDefinition.handler(mockClient, { id: 1 })).rejects.toThrow(
      'API error',
    );
  });
  ...
  it('should propagate API errors with status codes', async () => {
    ...
    await expect(getPersistedModelDefinition.handler(mockClient, { id: 999 })).rejects.toThrow(
      'Not Found',
    );
  });

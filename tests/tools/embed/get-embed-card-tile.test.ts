...
describe('getEmbedCardTile tool', () => {
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(getEmbedCardTileDefinition.handler(mockClient, input)).rejects.toThrow('API error');
  });

  it('should propagate API errors with status codes', async () => {
    const mockClient = createMockClientWithError('get', createApiError('Not Found', 404));
    await expect(getEmbedCardTileDefinition.handler(mockClient, input)).rejects.toThrow('Not Found');
  });

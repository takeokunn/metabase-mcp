...
describe('getEmbedDashboard tool', () => {
  ...
  it('should propagate client errors', async () => {
    ...
    await expect(getEmbedDashboardDefinition.handler(mockClient, input)).rejects.toThrow('API error');
  });

  it('should propagate API errors with status codes', async () => {
    const mockClient = createMockClientWithError('get', createApiError('Not Found', 404));
    await expect(getEmbedDashboardDefinition.handler(mockClient, input)).rejects.toThrow('Not Found');
  });

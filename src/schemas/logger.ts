...
export const GetLogsParamsSchema = z.object({
  level: z.string().optional().describe('Log level filter (e.g., "ERROR", "WARN", "INFO", "DEBUG")'),
  namespace: z.string().optional().describe('Logger namespace filter'),
  last_n_lines: z.number().int().positive().optional().describe('Number of recent log lines to return'),
});
...
export const CreateLogAdjustmentInputSchema = z.object({
  ...
  duration_ms: z.number().int().positive().describe('How long to apply the adjustment in milliseconds'),
});

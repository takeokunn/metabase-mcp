...
export const DatasetQuerySchema = z.object({
  ...
  query: z.record(z.string(), z.unknown()).optional().describe('MBQL query object'),
  native: z.record(z.string(), z.unknown()).optional().describe('Native SQL query object'),
});
...
export const ExecutePivotQueryInputSchema = z.object({
  query: z.record(z.string(), z.unknown()).describe('MBQL pivot query object'),
});

// Get native query input schema
export const GetNativeQueryInputSchema = z.object({
  query: z.record(z.string(), z.unknown()).describe('MBQL query to convert to native SQL'),
});

// Get query duration input schema
export const GetQueryDurationInputSchema = z.object({
  query: z.record(z.string(), z.unknown()).describe('MBQL query to estimate duration for'),
});

// Format query for visualization input schema
export const FormatQueryForVizInputSchema = z.object({
  query: z.record(z.string(), z.unknown()).describe('MBQL query to format for visualization'),
});

// Get dataset param values input schema
export const GetDatasetParamValuesInputSchema = z.object({
  parameter: z.record(z.string(), z.unknown()).describe('Parameter definition'),
...
export const GetDatasetQueryMetadataInputSchema = z.object({
  query: z.record(z.string(), z.unknown()).describe('The query to get metadata for'),
});

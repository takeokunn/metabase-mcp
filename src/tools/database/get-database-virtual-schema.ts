...
import { type GetDatabaseVirtualSchemaParams, GetDatabaseVirtualSchemaParamsSchema } from '@src/schemas/database';
...
export const getDatabaseVirtualSchemaDefinition: ToolDefinition<GetDatabaseVirtualSchemaParams> = {
  ...
  handler: async (client: MetabaseClient, input: GetDatabaseVirtualSchemaParams) => {
    const result = await client.get(`/api/database/${input.id}/datasets/${encodeURIComponent(input.schema)}`);

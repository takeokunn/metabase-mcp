...
import { type SearchDatasetParamValuesInput, SearchDatasetParamValuesInputSchema } from '@src/schemas/dataset';
...
export const searchDatasetParamValuesDefinition: ToolDefinition<SearchDatasetParamValuesInput> = {
  ...
  handler: async (client: MetabaseClient, input: SearchDatasetParamValuesInput) => {
    const result = await client.post(`/api/dataset/parameter/search/${input.query}`, { parameter: input.parameter, field_ids: input.field_ids });

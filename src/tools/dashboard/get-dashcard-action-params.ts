...
import { type GetDashcardActionParamsInput, GetDashcardActionParamsInputSchema } from '@src/schemas/dashboard';
...
export const getDashcardActionParamsDefinition: ToolDefinition<GetDashcardActionParamsInput> = {
  ...
  handler: async (client: MetabaseClient, input: GetDashcardActionParamsInput) => {
    const result = await client.get(`/api/dashboard/${input.dashboard_id}/dashcard/${input.dashcard_id}/execute`, input.parameters);

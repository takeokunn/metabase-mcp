...
import {
  type ExportDashcardQueryInput,
  ExportDashcardQueryInputSchema,
...
export const exportDashcardQueryDefinition: ToolDefinition<ExportDashcardQueryInput> = {
  ...
  handler: async (client: MetabaseClient, input: ExportDashcardQueryInput) => {
    const result = await client.post(
      `/api/dashboard/${input.dashboard_id}/dashcard/${input.dashcard_id}/card/${input.card_id}/query/${input.export_format}`,
